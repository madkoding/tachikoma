...
pub async fn list_playlists(
    ...
    let playlists = state.client.get_all_playlists().await.map_err(|e| {
    ...
    if query.include_songs.unwrap_or(false) {
        ...
        for playlist in playlists {
            let songs = state
                .client
                .get_songs_by_playlist(playlist.id)
                .await
                .unwrap_or_default();
...
pub async fn create_playlist(
    ...
    let playlist = state.client.create_playlist(data).await.map_err(|e| {
    ...
    state
        .event_broadcaster
        .broadcast(MusicEvent::PlaylistCreated(PlaylistEventData::from(
            &playlist,
        )));
...
pub async fn update_playlist(
    ...
    state
        .event_broadcaster
        .broadcast(MusicEvent::PlaylistUpdated(PlaylistEventData::from(
            &playlist,
        )));
...
pub async fn delete_playlist(
    ...
    let playlist = state
        ...
        .ok_or_else(|| {
            (
                ...
                Json(ErrorResponse::new("NOT_FOUND", "Playlist no encontrada")),
            )
        })?;
    ...
    if playlist.is_favorites {
        return Err((
            ...
            Json(ErrorResponse::new(
                "FORBIDDEN",
                "No puedes eliminar la playlist 'Me gusta'",
            )),
        ));
    }
    if playlist.is_suggestions {
        return Err((
            ...
            Json(ErrorResponse::new(
                "FORBIDDEN",
                "No puedes eliminar la playlist 'Sugerencias'",
            )),
        ));
    }

    let deleted = state.client.delete_playlist(id).await.map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
    ...
    if deleted {
        ...
        state
            .event_broadcaster
            .broadcast(MusicEvent::PlaylistDeleted { id: id.to_string() });
...
pub async fn add_song(
    ...
    let mut metadata = state
        .youtube
        .get_metadata(&data.youtube_url)
        .await
        .map_err(|e| {
            error!(error = %e, "Failed to get YouTube metadata");
            (
                StatusCode::BAD_REQUEST,
                Json(ErrorResponse::new("YOUTUBE_ERROR", e)),
            )
        })?;
    ...
    if let Ok(Some(_)) = state
        .client
        .get_song_by_youtube_id(&metadata.youtube_id, playlist_id)
        .await
    {
        return Err((
            StatusCode::CONFLICT,
    ...
    let enriched = state
        .metadata_enricher
        .enrich(&metadata.title, metadata.artist.as_deref())
        .await;
    ...
    if create_data.cover_url.is_none() {
        let cover = state
            .cover_art
            .search_cover(&metadata.title, metadata.artist.as_deref())
            .await;
    ...
    let song = state
        .client
        .create_song(playlist_id, create_data, metadata)
        .await
        .map_err(|e| {
...
pub async fn update_song(
    ...
    state
        .event_broadcaster
        .broadcast(MusicEvent::SongUpdated(SongEventData::from(&updated)));
...
pub async fn delete_song(
    ...
    let deleted = state.client.delete_song(song_id).await.map_err(|e| {
...
pub async fn stream_song(
    ...
    let stream_info = state
        .youtube
        .get_audio_stream_url(&song.youtube_id)
        .await
        .map_err(|e| {
    ...
    let child = Command::new(&state.config.ffmpeg_path)
        .args([
            ...
            "2",
            "-af",
            "highpass=f=25,loudnorm=I=-14:TP=-1:LRA=11,alimiter=limit=0.95:level=false",
            "-f",
            "ogg",
            "-",
        ])
...
pub async fn search_youtube_enriched(
    ...
    let enriched = state
        .metadata_enricher
        .enrich_search_results(to_enrich)
        .await;
...
pub async fn enrich_metadata(
    ...
    let enriched = state
        .metadata_enricher
        .enrich(&request.title, request.channel.as_deref())
        .await;
...
pub async fn get_youtube_metadata(
    ...
    let metadata = state.youtube.get_metadata(&query.url).await.map_err(|e| {
...
pub async fn search_cover(
    ...
    let result = state
        .cover_art
        .search_cover(&query.title, query.artist.as_deref())
        .await;
...
pub async fn get_equalizer(State(state): State<Arc<AppState>>) -> Json<EqualizerSettings> {
    let settings = state
        .client
        .get_equalizer_settings()
        .await
        .unwrap_or_else(|e| {
...
pub async fn update_equalizer(
    ...
    state
        .client
        .save_equalizer_settings(settings.clone())
        .await
        .map_err(|e| {
...
pub async fn get_listening_history(
    ...
    let history = state
        .client
        .get_listening_history(query.limit.unwrap_or(50))
        .await
        .map_err(|e| {
...
pub async fn get_most_played(
    ...
    let songs = state
        .client
        .get_most_played_songs(query.limit.unwrap_or(20))
        .await
        .map_err(|e| {
...
pub async fn init_special_playlists(
    ...
    let favorites = state
        .client
        .get_or_create_favorites_playlist()
        .await
        .map_err(|e| {
    ...
    let suggestions = state
        .client
        .get_or_create_suggestions_playlist()
        .await
        .map_err(|e| {
...
pub async fn toggle_song_like(
    ...
    let song = state
        ...
        .ok_or_else(|| {
            (
                StatusCode::NOT_FOUND,
                Json(ErrorResponse::new("NOT_FOUND", "Canción no encontrada")),
            )
        })?;
    ...
    let updated_song = state
        .client
        .update_song(song_id, update)
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let favorites = state
        ...
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse::new("FAVORITES_ERROR", e.to_string())),
            )
        })?;

    if new_liked_status {
        // Add to favorites if not already there
        let favorites_songs = state
            .client
            .get_songs_by_playlist(favorites.id)
            .await
            .unwrap_or_default();

        let already_in_favorites = favorites_songs
            .iter()
            .any(|s| s.youtube_id == updated_song.youtube_id);
        ...
        if !already_in_favorites {
            ...
            if let Err(e) = state
                .client
                .create_song(favorites.id, create_data, metadata)
                .await
            {
        ...
        if let Some(fav_song) = favorites_songs
            .iter()
            .find(|s| s.youtube_id == updated_song.youtube_id)
        {
...
pub async fn fetch_song_cover(
    ...
    let cover = state
        .cover_art
        .search_cover(&song.title, song.artist.as_deref())
        .await;
...
pub async fn get_liked_songs(
    ...
    let songs = state.client.get_liked_songs().await.map_err(|e| {
...
pub async fn refresh_suggestions(
    ...
    let suggestions = state
        ...
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse::new("SUGGESTIONS_ERROR", e.to_string())),
            )
        })?;

    // Get top played songs to base suggestions on
    let top_songs = state.client.get_most_played_songs(10).await.map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse::new("TOP_SONGS_ERROR", e.to_string())),
        )
    })?;
    ...
    if top_songs.is_empty() {
        return Err((
            ...
            Json(ErrorResponse::new(
                "NO_HISTORY",
                "No hay suficiente historial para generar sugerencias. Escucha más música primero.",
            )),
        ));
    }
    ...
    if let Err(e) = state
        .client
        .update_suggestions_timestamp(suggestions.id)
        .await
    {
    ...
    let updated_playlist = state
        .client
        .get_playlist_with_songs(suggestions.id)
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse::new("GET_ERROR", e.to_string())),
            )
        })?
        .ok_or_else(|| {
            (
                ...
                Json(ErrorResponse::new(
                    "NOT_FOUND",
                    "Playlist de sugerencias no encontrada",
                )),
            )
        })?;
