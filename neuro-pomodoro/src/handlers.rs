...
pub async fn get_timer_state(
    ...
    let completed_pomodoros = sessions
        .iter()
        .filter(|s| s.session_type == SessionType::Work && s.status == SessionStatus::Completed)
        .count() as u32;

    let total_work_minutes = sessions
        .iter()
        .filter(|s| s.session_type == SessionType::Work && s.status == SessionStatus::Completed)
        .map(|s| s.duration_minutes)
        .sum();

    let total_break_minutes = sessions
        .iter()
        .filter(|s| {
            matches!(
                s.session_type,
...
pub async fn get_stats(
    ...
    let end =
        NaiveDate::parse_from_str(&end_str, "%Y-%m-%d").unwrap_or_else(|_| Utc::now().date_naive());
...
pub async fn pause_active_session(
    ...
    let active_session = sessions.into_iter().find(|s| {
        matches!(
            s.status,
            SessionStatus::Working | SessionStatus::ShortBreak | SessionStatus::LongBreak
        )
    });
...
pub async fn resume_active_session(
    ...
    let paused_session = sessions
        .into_iter()
        .find(|s| s.status == SessionStatus::Paused);
...
pub async fn update_settings(
    ...
    if let Some(v) = request.work_duration_minutes {
        settings.work_duration_minutes = v;
    }
    if let Some(v) = request.short_break_minutes {
        settings.short_break_minutes = v;
    }
    if let Some(v) = request.long_break_minutes {
        settings.long_break_minutes = v;
    }
    if let Some(v) = request.sessions_until_long_break {
        settings.sessions_until_long_break = v;
    }
    if let Some(v) = request.auto_start_breaks {
        settings.auto_start_breaks = v;
    }
    if let Some(v) = request.auto_start_work {
        settings.auto_start_work = v;
    }
